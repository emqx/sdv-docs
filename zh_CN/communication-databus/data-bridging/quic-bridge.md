...
MQTT 消息服务器 已支持 MQTT over QUIC 桥接，用户可以使用 QUIC 作为 MQTT 协议的传输层来与 EMQX 5.0 消息服务建立桥接进行数据同步，从而为无法集成或找到合适的 MQTT over QUIC SDK 的端侧设备和难以修改固件的嵌入式设备提供在 IoT 场景利用 QUIC 协议优势的捷径。依靠 EMQX + SDV-Flow 的云边一体化的消息架构，用户能够快速且低成本的在泛物联网场景中完成跨时空地域的数据采集和同步需求。
...
![Multi-stream](assets/multi-stream2.png)

## 启用 MQTT over QUIC 桥接

MQTT 消息服务器 的 QUIC 模组处于默认关闭状态，如希望使用 MQTT over QUIC 桥接，请通过[编译方式安装 MQTT 消息服务器](../installation/build-options.md)，并在编译时启用 QUIC 模组：
...
如使用 Hocon 版本配置项 MQTT 消息服务器 版本在 0.19 及以上，除将相关配置直接写入  `nanomq.conf ` 中外，您也可单独为桥接定义一份配置文件，如 `nanomq_bridge.conf` ，然后通过 HOCON 的 `include` 语法在 `nanomq.conf` 中引用此文件：
...
本节将通过 MQTT 消息服务器 自带的客户端工具测试测试新建的 MQTT over QUIC 桥接，我们将新建 2 个连接，分别连接到 NanoMQ 和 MQTT over QUIC 数据桥接，用于验证 MQTT 消息服务器 和数据桥接的消息收发服务。

### 测试消息转发

1. 为远端 EMQX Broker 订阅消息主题：

   为 **EMQX** 订阅转发主题 “`forward1/#`”，用于接收由 **MQTT 消息服务器 **转发的数据：
...
2. 新建一个命令行窗口，发布消息到 **MQTT 消息服务器** Broker，主题为 “`forward1/msg`”：

   ```bash
   $ ./nanomq_cli pub -h "local.broker.address"  -t "forward1/msg" -m "forward_msg" -q 2
   ```

3. 返回第一个命令行窗口，可以看到由 MQTT 消息服务器 转发到消息，例如：
...
1. 为本地 MQTT 消息服务器 订阅消息主题：

   为 **MQTT 消息服务器** 订阅主题 “`cmd/topic1`”，用于接收 **EMQX** 发布的数据：
...
QUIC 协议相较于 TCP 的一大优势在于解决了队首阻塞的问题，但这是依赖于 QUIC 的单链接多 Stream 特性的。针对网络拥塞或者网络抖动等情况，SDV-Flow 的 MQTT 消息服务器 和 EMQX 5.0 一起设计和引入了 Mutli-stream QUIC 协议标准，以提供更好消息传输体验。

![MQTT 消息服务器 多流桥接](./assets/multi-stream.png)
...
之后 MQTT 消息服务器 就会自动根据 Topic 将数据包导流至不同的 Stream 发送。经过内部测试，在使用模拟 2s 延迟和 40% 丢包的弱网环境时，多流桥接可以显著降低延时。
