...
## MQTT 消息服务器配置
MQTT 消息服务器 (又称为 NanoMQ) 的默认配置文件格式是 HOCON 。 HOCON（ Human-Optimized Config Object Notation ）是一个 JSON 的超集，非常适用于易于人类读写的配置数据存储。你可以在 etc 目录找到这些配置文件，周边功能如鉴权/网关等可以使用独立的配置文件（通过 Include 方式指定路径），主要配置文件包括：
...
这种格式与 'cuttlefish' 格式并不是完全兼容，因为 HOCON 经常要求字符串两端加上引号。
...
# 环境变量
MQTT消息服务器 也支持通过环境变量自定义配置，支持的环境变量列表如下：
