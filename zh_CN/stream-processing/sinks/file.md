...
| rollingHook        | 是    | 定义 [rolling 策略](#rolling-策略)的属性之一。指定滚动文件时调用的动作，当前仅支持配置为 `nano`，用于通知 Nano MQ 上传文件 |
|
| rollingHook        | 是    | 定义 [rolling 策略](#rolling-策略)的属性之一。指定滚动文件时调用的动作，当前仅支持配置为 `nano`，用于通知 Nano MQ 上传文件 |
|
| rollingHookProps   | 是    | 定义 [rolling 策略](#rolling-策略)的属性之一。指定 rollingHook 的额外参数。每种 rollingHook 可能支持不同的参数。 |
|
...
当文件滚动时，可配置额外的动作。目前仅支持 `nano` 属性，用于通知 Nano MQ 上传已滚动的文件。可通过 rollingHookProps 定义 hook 的属性。nano hook 支持如下属性：

- topic： 配置通知采用的 MQTT 主题
- delete：配置延迟删除时间
...
若需要文件滚动时，通知 Nano MQ 上传并在上传后删除，可配置如下：
